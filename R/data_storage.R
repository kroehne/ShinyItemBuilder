...
shinyassess_internal_create_or_load_session <- function (session){
  if (!exists(session$userData$cbasession, runtime.data)){
    ...
    if (file.exists(paste0("data/",session$userData$cbasession,".RDS" ))){
      
      runtime.data[[session$userData$cbasession]] <<- readRDS(file=file.path("data",paste0(session$userData$cbasession,".RDS")))
...
shinyassess_internal_save_session <- function(session){
  saveRDS(runtime.data[[session$userData$cbasession]], file=file.path("data",paste0(session$userData$cbasession,".RDS")))
}
